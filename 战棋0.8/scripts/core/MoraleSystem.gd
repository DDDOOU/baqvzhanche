...
func tick_turn(_turn: int) -> void:
	"""每回合士气微调"""
	for stored_id in unit_morale.keys():
		# JSON 对象的键读取后会变成 String；士气接口统一使用整数单位ID。
		var unit_id := int(stored_id)
		var tier = get_unit_morale_tier(unit_id)
		match tier:
			MoraleTier.BROKEN:
				# 崩溃单位有小概率自然恢复
...
func deserialize(data: Dictionary) -> void:
	# JSON 存档会把 Dictionary 的整数键序列化为字符串，读取时在这里恢复。
	unit_morale.clear()
	var stored: Dictionary = data.get("unit_morale", {})
	for stored_id in stored.keys():
		unit_morale[int(stored_id)] = stored[stored_id].duplicate(true)
