...
const FADE_IN_SECONDS := 0.50
const HOLD_SECONDS := 5.0
const FADE_OUT_SECONDS := 1.75
...
func _ready() -> void:
	...
	# 与单位属性配置界面保持一致：不依赖父节点的最小尺寸，直接以整个视口布局。
...
func _build_interface() -> void:
	...
	backdrop.color = Color(0.018, 0.022, 0.025, 0.88)
	...
	frame_style.bg_color = Color(0.025, 0.030, 0.032, 0.90)
	...
	credits_grid.size_flags_horizontal = Control.SIZE_SHRINK_CENTER
	credits_grid.mouse_filter = Control.MOUSE_FILTER_IGNORE
	var credits_grid_center := CenterContainer.new()
	credits_grid_center.name = "CreditsGridCenter"
	credits_grid_center.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	credits_grid_center.mouse_filter = Control.MOUSE_FILTER_IGNORE
	content.add_child(credits_grid_center)
	credits_grid_center.add_child(credits_grid)

	for row in CREDIT_ROWS:
		var people := _make_label(String(row[0]), 18, Color(0.91, 0.91, 0.86), HORIZONTAL_ALIGNMENT_RIGHT)
		people.custom_minimum_size = Vector2(420, 30)
		credits_grid.add_child(people)
		var role := _make_label(String(row[1]), 18, Color(0.92, 0.64, 0.30), HORIZONTAL_ALIGNMENT_LEFT)
		role.custom_minimum_size = Vector2(420, 30)
...
func get_fade_durations() -> Vector2:
	return Vector2(FADE_IN_SECONDS, FADE_OUT_SECONDS)
