...
# 建筑遮挡单位时保留较多细节，只做轻度虚化。
const OCCLUSION_ALPHA := 0.58
...
func set_unit_occlusion_faded(faded: bool) -> void:
	"""单位被建筑图片挡住时，只虚化图片，不改变建筑占地或碰撞规则。"""
	var visual := _get_visual()
	if visual == null:
		return
	var target_alpha := OCCLUSION_ALPHA if faded else 1.0
	if not is_equal_approx(visual.modulate.a, target_alpha):
		visual.modulate.a = target_alpha


func occludes_screen_point(screen_point: Vector2) -> bool:
	"""判断屏幕点是否落在建筑图片范围内，供单位遮挡虚化使用。"""
	var visual := _get_visual()
	if visual == null or visual.texture == null or not visual.visible:
		return false
	var local_rect := visual.get_rect()
	...
	var screen_rect := Rect2(corners[0], Vector2.ZERO)
	for corner in corners:
		screen_rect = screen_rect.expand(corner)
	# 略微收紧边框，避免单位只碰到透明图片边缘时也触发虚化。
	return screen_rect.grow(-2.0).has_point(screen_point)
