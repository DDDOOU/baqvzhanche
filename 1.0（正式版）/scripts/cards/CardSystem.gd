...
var sacrifice_aftereffects: Dictionary = {} # unit_id → 战后剩余生命倍率
...
func get_card_use_error(card_index: int, target_col: int, target_row: int) -> String:
	...
	if card.card_id == "sacrifice_charge":
		var cell = GridManager.get_cell(target_col, target_row)
		if cell == null or cell.occupant_unit == null \
				or cell.occupant_unit.faction != UnitBase.Faction.WARSAW_PACT:
			return "牺牲冲锋只能指定一个己方单位。"
	return ""
...
func _execute_card_effect(card: CardInstance, target_col: int, target_row: int) -> void:
	...
	match card.card_id:
		...
		"smoke_screen":
			# 烟雾遮障：4×4范围烟雾
			# 延迟效果会在本回合结算前生成并立即衰减一次，因此初始记为2，
			# 确保卡面所写的“下回合”仍有完整一回合效果。
		...
		"sacrifice_charge":
			...
			if cell and cell.occupant_unit and cell.occupant_unit.faction == UnitBase.Faction.WARSAW_PACT:
				...
				sacrifice_aftereffects[unit.unit_id] = 0.5
				BattleLog.add_log("[卡牌] 牺牲冲锋: 「%s」本回合伤害×1.5，战后损失50%%当前生命" % unit.unit_name, Color(1.0, 0.4, 0.2))
...
func resolve_pending_card_effects() -> void:
	...
	for entry in pending_card_effects:
		...
		match cid:
			...
			"smoke_screen":
				# 烟雾遮障：在标记区域施放烟雾
				CombatSystem.apply_smoke(tc, tr, 2, 4)
...
func tick_cooldowns() -> void:
	...
	# 牺牲冲锋在本回合攻击完成后支付生命代价，再清除伤害加成。
	for uid in sacrifice_aftereffects.keys():
		var unit := _get_unit_by_id(int(uid))
		if unit and unit.is_alive:
			unit.current_health = maxf(1.0,
				unit.current_health * float(sacrifice_aftereffects[uid]))
			BattleLog.add_log("[牺牲冲锋] 「%s」战后生命降至%.0f" % [
				unit.unit_name, unit.current_health], Color(1.0, 0.45, 0.25))
	sacrifice_aftereffects.clear()
	sacrifice_buffs.clear()
...
func serialize() -> Dictionary:
	return {
		...
		"sacrifice_aftereffects": sacrifice_aftereffects,
...
func deserialize(data: Dictionary) -> void:
	...
	sacrifice_buffs.clear()
	for stored_id in data.get("sacrifice_buffs", {}).keys():
		sacrifice_buffs[int(stored_id)] = float(data["sacrifice_buffs"][stored_id])
	sacrifice_aftereffects.clear()
	for stored_id in data.get("sacrifice_aftereffects", {}).keys():
		sacrifice_aftereffects[int(stored_id)] = float(data["sacrifice_aftereffects"][stored_id])
	# JSON对象键会变成字符串；恢复为整数ID，并把断电命中惩罚重新应用到已重建单位。
	power_cut_units.clear()
	for stored_id in data.get("power_cut_units", {}).keys():
		var uid := int(stored_id)
		var entry: Dictionary = data["power_cut_units"][stored_id].duplicate(true)
		power_cut_units[uid] = entry
		var unit := _get_unit_by_id(uid)
		if unit and unit.is_alive and entry.has("orig_accuracy"):
			unit.accuracy = float(entry["orig_accuracy"]) * 0.5
...
func _cards_from_state(states: Array) -> Array[CardInstance]:
	...
	for s in states:
		...
		if s is Dictionary:
			...
			# 兼容旧格式（元素为纯 id 字符串）
			c.card_id = String(s)
		# 新旧存档格式都只保存动态状态；名称、费用、描述等必须统一从数据库回填。
		_fill_card_info(c)
		out.append(c)
	return out
