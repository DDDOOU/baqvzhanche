...
## 每个行动槽至少展示这么久。待命单位也会在地图和顺序条上被看见，
## 有长距离移动的单位以实际动画时长为准，不重复追加等待。
const MIN_ACTION_PRESENTATION_SECONDS := 0.50
...
func start_execution_phase() -> void:
	...
	for order_index in range(action_queue.size()):
		...
		if unit == null or not unit.is_alive:
			continue
		var action_started_at := Time.get_ticks_msec()
		unit_action_started.emit(unit.unit_id, order_index, action_queue.size())
		await _execute_unit_action(unit)
		# 无移动/无目标单位原本会在同一帧内连续完成，导致顺序条频繁重建动画，
		# 同时玩家也看不清是谁选择待命。补足最短展示时间，并自然让出主线程。
		var elapsed_seconds := float(Time.get_ticks_msec() - action_started_at) / 1000.0
		var remaining_presentation := MIN_ACTION_PRESENTATION_SECONDS - elapsed_seconds
		if remaining_presentation > 0.0:
			await get_tree().create_timer(remaining_presentation).timeout
