...
class TileCell:
	...
	func get_effective_height() -> int:
		# TileSet 自定义数据 height 是关卡设计者填写的最终高度。
		# 不再叠加地形类型的旧高度，避免水面/山地被重复计算。
		return clampi(height, -2, 2)

	func get_vision_modifier() -> int:
		# 高处扩大视野，洼地缩小视野。
		return get_effective_height()

	func get_attack_range_modifier() -> int:
		# 最高点获得1格射程，最低洼地损失1格；中间等级不改射程。
		var level := get_effective_height()
		if level >= 2:
			return 1
		if level <= -2:
			return -1
	...
	func get_concealment() -> float:
		var base_concealment: float = custom_concealment if custom_concealment >= 0.0 \
			else float(TERRAIN_PROPERTIES[terrain]["concealment"])
		# 洼地更难被观察：-1/+10%，-2/+20%；高地不额外提供隐蔽。
...
func _apply_map_data(data: Dictionary) -> void:
	for cell_data in data.get("terrain", []):
		...
		if cell:
			...
			cell.height = clampi(int(cell_data.get("height", 0)), -2, 2)
