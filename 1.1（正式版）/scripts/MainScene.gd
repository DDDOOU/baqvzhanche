...
var victory_progress_toggle_button: Button
var is_victory_progress_collapsed := false
...
var opening_dialogue_layer: CanvasLayer = null # 战前剧情之前的开场对白层
var opening_dialogue_panel: Panel = null
var opening_dialogue_portrait_left: Control = null
var opening_dialogue_portrait_right: Control = null
...
var _opening_dialogue_entries: Array = []
var _opening_dialogue_index: int = 0
var _opening_dialogue_speaker: Label = null
var _opening_dialogue_body: Label = null
var _opening_dialogue_next_button: Button = null
var _opening_portrait_left_texture: TextureRect = null
var _opening_portrait_right_texture: TextureRect = null
...
var building_occlusion_elapsed: float = 0.0
...
const BUILDING_OCCLUSION_INTERVAL: float = 0.10
...
func _create_victory_progress_panel(hud: CanvasLayer) -> void:
	...
	var objective_style := _make_translucent_panel_stylebox()
	objective_style.bg_color = Color(0.055, 0.045, 0.025, 0.92)
	objective_style.border_color = Color(1.0, 0.72, 0.18, 0.96)
	objective_style.set_border_width_all(2)
	victory_progress_label.add_theme_stylebox_override("normal", objective_style)
	victory_progress_label.add_theme_font_size_override("normal_font_size", 14)
	hud.add_child(victory_progress_label)

	victory_progress_toggle_button = Button.new()
	victory_progress_toggle_button.name = "VictoryProgressToggle"
	victory_progress_toggle_button.tooltip_text = "收起或展开左侧关卡目标"
	victory_progress_toggle_button.pressed.connect(_on_victory_progress_toggle_pressed)
	_style_hud_button(victory_progress_toggle_button,
		Color(0.16, 0.12, 0.05, 0.96), Color(1.0, 0.72, 0.18, 0.96))
	hud.add_child(victory_progress_toggle_button)
...
func _layout_victory_progress_panel() -> void:
	if victory_progress_label == null or victory_progress_toggle_button == null:
		return
	var vp := get_viewport_rect().size
	var panel_w := minf(430.0, maxf(320.0, vp.x * 0.33))
	victory_progress_label.position = Vector2(10, 204)
	victory_progress_label.size = Vector2(panel_w, 126)
	victory_progress_label.visible = not is_victory_progress_collapsed
	if is_victory_progress_collapsed:
		victory_progress_toggle_button.position = Vector2(10, 204)
		victory_progress_toggle_button.size = Vector2(104, 32)
		victory_progress_toggle_button.text = "展开目标"
	else:
		victory_progress_toggle_button.position = Vector2(10 + panel_w - 80, 208)
		victory_progress_toggle_button.size = Vector2(74, 28)
		victory_progress_toggle_button.text = "收起"


func _on_victory_progress_toggle_pressed() -> void:
	is_victory_progress_collapsed = not is_victory_progress_collapsed
	_layout_victory_progress_panel()
...
func _process(delta: float) -> void:
	...
	building_occlusion_elapsed += delta
	if building_occlusion_elapsed >= BUILDING_OCCLUSION_INTERVAL:
		building_occlusion_elapsed = 0.0
...
func _update_victory_progress() -> void:
	...
	var objective := "终局控制至少2个VP"
	var mechanic := "地图旗帜表示战略VP"
	if level_data != null:
		objective = String(level_data.primary_objective)
		mechanic = _get_core_mechanics_plain(level_data, 1)
	victory_progress_label.text = "[font_size=16][color=#ffd65a][b]★ 核心胜利条件[/b][/color]\n[color=#fff2bd][b]%s[/b][/color][/font_size]\n[font_size=14][color=#d8d8d8]进度[/color] 第%d/%d回合　VP [color=#69b9ff]我方%d[/color]/%d　[color=#ff766c]敌方%d[/color]　中立%d\n[color=#f0bd55][b]本关机制[/b][/color] %s\n[color=#d8d8d8]兵力[/color] 我方%d　敌方%d[/font_size]" % [
		objective, TurnManager.current_turn, VictoryManager.max_turns,
		vp.wp, vp_total, vp.nato, vp.neutral, mechanic,
		counts.wp, counts.nato]


func _get_core_mechanics_plain(level_data, max_lines: int = -1) -> String:
	if level_data == null or level_data.core_mechanics.is_empty():
		return "本关无额外机制"
	var lines: Array[String] = []
	var limit: int = level_data.core_mechanics.size() if max_lines < 0 else mini(max_lines, level_data.core_mechanics.size())
	for index in range(limit):
		lines.append(String(level_data.core_mechanics[index]))
	return "　".join(lines)


func _get_core_mechanics_bbcode(level_data, max_lines: int = -1) -> String:
	if level_data == null or level_data.core_mechanics.is_empty():
		return "[color=#c8c8c8]本关无额外机制[/color]"
	var lines: Array[String] = []
	var limit: int = level_data.core_mechanics.size() if max_lines < 0 else mini(max_lines, level_data.core_mechanics.size())
	for index in range(limit):
		lines.append("[color=#f4c65d]◆[/color] %s" % String(level_data.core_mechanics[index]))
	return "\n".join(lines)
...
func _on_level_started(level_id: int) -> void:
	...
	# 目标与特殊机制同时写入右侧战报，确保玩家关闭开场字幕后仍可回看。
	BattleLog.add_log("【核心胜利条件】%s" % ld.primary_objective, Color(1.0, 0.82, 0.25))
	for mechanic in ld.core_mechanics:
		BattleLog.add_log("【本关机制】%s" % String(mechanic), Color(0.95, 0.68, 0.28))
...
func _restore_unit_vision() -> void:
	...
	# 第4回合恢复数值后立即重算己方联合视野，不要求玩家先移动一格才看到变化。
	FogOfWar.refresh()
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventKey and event.pressed and not event.echo:
		...
		return
	# 开场对白、战前剧情或任务要求打开时，只让 GUI 接收鼠标，避免误触战场。
	if opening_dialogue_layer or intro_story_layer or mission_briefing_layer:
		return
...
func _handle_key_input(event: InputEventKey) -> void:
	if event.is_action_pressed("toggle_fullscreen") or (event.alt_pressed and event.keycode in [KEY_ENTER, KEY_KP_ENTER]):
		...
		get_viewport().set_input_as_handled()
		return
	if opening_dialogue_layer and event.keycode in [KEY_ENTER, KEY_KP_ENTER, KEY_SPACE, KEY_ESCAPE]:
		_on_opening_dialogue_next_pressed()
		get_viewport().set_input_as_handled()
		return
...
func _is_pointer_over_button(pos: Vector2) -> bool:
	var buttons: Array = [finish_planning_button, card_toggle_button, pause_button,
		confirm_move_button, cancel_move_button, loan_button, victory_progress_toggle_button]
...
func _on_planning_started(turn: int) -> void:
	...
	# 回合结算可能让烟雾消散或VP易手；进入新计划阶段时刷新对应视觉状态。
	unit_renderer.queue_redraw()
	tile_grid.queue_redraw()
...
func _on_card_effect_resolved(card_id: String, target_col: int, target_row: int) -> void:
	...
	# 烟雾会改变区域内单位的透明度；数值效果生效后必须立即刷新单位层。
	if card_id == "smoke_screen":
		unit_renderer.queue_redraw()
...
func _show_briefing_panel() -> void:
	...
	var panel = Panel.new()
	panel.custom_minimum_size = Vector2(720, 610)
	...
	info.custom_minimum_size = Vector2(0, 240)
	info.add_theme_font_size_override("normal_font_size", 15)
	var info_text := "[font_size=20][color=#ffd65a][b]★ 核心胜利条件[/b][/color][/font_size]\n[color=#fff2bd][b]%s[/b][/color]\n[color=#d8cfa9]%s[/color]\n\n[color=#f4c65d][b]本关特殊机制[/b][/color]\n%s\n\n[color=#ff9b85][b]失败条件[/b][/color] %s\n[color=#c9ced8]回合限制：%d 回合[/color]" % [
		level_data.primary_objective, level_data.victory_condition,
		_get_core_mechanics_bbcode(level_data), level_data.failure_condition, level_data.max_turns]
...
func _show_opening_dialogue() -> void:
	"""在战前剧情之前播放开场对白，并为后续人物立绘保留画面空间。"""
	if opening_dialogue_layer:
		return
	var level_data = LevelDatabase.get_level(GameManager.current_level_id)
	if level_data == null:
		GameManager.confirm_intro()
		return
	_opening_dialogue_entries = level_data.opening_dialogue.duplicate(true)
	if _opening_dialogue_entries.is_empty():
		_show_opening_story_dialog()
		return
	_opening_dialogue_index = 0
	_set_gameplay_ui_visible(false)

	var layer := CanvasLayer.new()
	layer.layer = 90
	layer.name = "OpeningDialogueLayer"
	add_child(layer)
	opening_dialogue_layer = layer

	var input_blocker := ColorRect.new()
	input_blocker.name = "OpeningDialogueInputBlocker"
	...
	opening_dialogue_portrait_left = Control.new()
	opening_dialogue_portrait_left.name = "OpeningPortraitLeft"
	opening_dialogue_portrait_left.anchor_left = 0.04
	opening_dialogue_portrait_left.anchor_top = 0.08
	opening_dialogue_portrait_left.anchor_right = 0.30
	opening_dialogue_portrait_left.anchor_bottom = 0.72
	opening_dialogue_portrait_left.mouse_filter = Control.MOUSE_FILTER_IGNORE
	layer.add_child(opening_dialogue_portrait_left)
	_opening_portrait_left_texture = CharacterPortraits.create_slot("PortraitTexture")
	_opening_portrait_left_texture.set_anchors_preset(Control.PRESET_FULL_RECT)
	opening_dialogue_portrait_left.add_child(_opening_portrait_left_texture)

	opening_dialogue_portrait_right = Control.new()
	opening_dialogue_portrait_right.name = "OpeningPortraitRight"
	opening_dialogue_portrait_right.anchor_left = 0.70
	opening_dialogue_portrait_right.anchor_top = 0.08
	opening_dialogue_portrait_right.anchor_right = 0.96
	opening_dialogue_portrait_right.anchor_bottom = 0.72
	opening_dialogue_portrait_right.mouse_filter = Control.MOUSE_FILTER_IGNORE
	layer.add_child(opening_dialogue_portrait_right)
	_opening_portrait_right_texture = CharacterPortraits.create_slot("PortraitTexture")
	_opening_portrait_right_texture.set_anchors_preset(Control.PRESET_FULL_RECT)
	opening_dialogue_portrait_right.add_child(_opening_portrait_right_texture)

	var bottom_center := CenterContainer.new()
	bottom_center.name = "OpeningDialogueBottomCenter"
	bottom_center.anchor_left = 0.0
	bottom_center.anchor_top = 1.0
	bottom_center.anchor_right = 1.0
	bottom_center.anchor_bottom = 1.0
	bottom_center.offset_top = -260.0
	bottom_center.offset_bottom = -24.0
	layer.add_child(bottom_center)

	opening_dialogue_panel = Panel.new()
	opening_dialogue_panel.name = "OpeningDialoguePanel"
	opening_dialogue_panel.custom_minimum_size = Vector2(980, 216)
	var panel_style := StyleBoxFlat.new()
	panel_style.bg_color = Color(0.22, 0.23, 0.24, 0.40)
	panel_style.border_width_left = 1
	panel_style.border_width_right = 1
	panel_style.border_width_top = 1
	panel_style.border_width_bottom = 1
	panel_style.border_color = Color(0.78, 0.80, 0.82, 0.35)
	panel_style.corner_radius_top_left = 6
	panel_style.corner_radius_top_right = 6
	panel_style.corner_radius_bottom_left = 6
	panel_style.corner_radius_bottom_right = 6
	opening_dialogue_panel.add_theme_stylebox_override("panel", panel_style)
	bottom_center.add_child(opening_dialogue_panel)
	...
	opening_dialogue_panel.add_child(content)

	_opening_dialogue_speaker = Label.new()
	_opening_dialogue_speaker.add_theme_font_size_override("font_size", 20)
	_opening_dialogue_speaker.add_theme_color_override("font_color", Color(0.96, 0.84, 0.52))
	content.add_child(_opening_dialogue_speaker)

	_opening_dialogue_body = Label.new()
	_opening_dialogue_body.custom_minimum_size = Vector2(0, 96)
	_opening_dialogue_body.vertical_alignment = VERTICAL_ALIGNMENT_CENTER
	_opening_dialogue_body.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	_opening_dialogue_body.add_theme_font_size_override("font_size", 19)
	_opening_dialogue_body.add_theme_color_override("font_color", Color(0.97, 0.97, 0.97))
	content.add_child(_opening_dialogue_body)
	...
	_opening_dialogue_next_button = Button.new()
	_opening_dialogue_next_button.custom_minimum_size = Vector2(170, 42)
	_opening_dialogue_next_button.pressed.connect(_on_opening_dialogue_next_pressed)
	actions.add_child(_opening_dialogue_next_button)
	_render_opening_dialogue_entry()


func _render_opening_dialogue_entry() -> void:
	if _opening_dialogue_entries.is_empty() or _opening_dialogue_index >= _opening_dialogue_entries.size():
		return
	var entry = _opening_dialogue_entries[_opening_dialogue_index] as Dictionary
	_opening_dialogue_speaker.text = String(entry.get("speaker", "战区记录"))
	_opening_dialogue_body.text = String(entry.get("text", ""))
	CharacterPortraits.show_speaker(
		_opening_portrait_left_texture, _opening_portrait_right_texture,
		String(entry.get("speaker", "战区记录")), String(entry.get("text", "")))
	_opening_dialogue_next_button.text = (
		"进入战前剧情" if _opening_dialogue_index == _opening_dialogue_entries.size() - 1 else "继续"
	)


func _on_opening_dialogue_next_pressed() -> void:
	if _opening_dialogue_index + 1 < _opening_dialogue_entries.size():
		_opening_dialogue_index += 1
		_render_opening_dialogue_entry()
		return
	_close_opening_dialogue()
	_show_opening_story_dialog()
...
func _close_opening_dialogue() -> void:
	if opening_dialogue_layer:
		opening_dialogue_layer.queue_free()
		opening_dialogue_layer = null
	opening_dialogue_panel = null
	opening_dialogue_portrait_left = null
	opening_dialogue_portrait_right = null
	_opening_portrait_left_texture = null
	_opening_portrait_right_texture = null
	_opening_dialogue_entries.clear()
	_opening_dialogue_index = 0
	_opening_dialogue_speaker = null
	_opening_dialogue_body = null
	_opening_dialogue_next_button = null
...
func _show_opening_story_dialog() -> void:
	...
	var panel = Panel.new()
	panel.custom_minimum_size = Vector2(780, 470)
	var style := StyleBoxFlat.new()
	...
	var mechanics := RichTextLabel.new()
	mechanics.bbcode_enabled = true
	mechanics.fit_content = true
	mechanics.scroll_active = false
	mechanics.custom_minimum_size = Vector2(0, 110)
	mechanics.add_theme_font_size_override("normal_font_size", 16)
	mechanics.text = "[center][color=#ffd65a][b]本关核心机制[/b][/color][/center]\n%s" % _get_core_mechanics_bbcode(level_data)
	vbox.add_child(mechanics)
...
func _show_mission_briefing_dialog() -> void:
	...
	body.text = "[center][font_size=23][color=#ffd65a][b]★ 核心胜利条件[/b][/color][/font_size]\n[color=#fff2bd][b]%s[/b][/color]\n[color=#d8cfa9]%s[/color][/center]\n\n[color=#f4c65d][b]本关特殊机制[/b][/color]\n%s\n\n[color=#ff9b85][b]失败条件[/b][/color] %s\n[color=#c9ced8]回合限制：%d 回合[/color]" % [
		level_data.primary_objective,
		level_data.victory_condition,
		_get_core_mechanics_bbcode(level_data),
...
func _on_unit_step(_uid: int, _col: int, _row: int) -> void:
	# UnitBase.set_grid_position 已按实际经过的路径格增量更新迷雾。
	# 这里仅保证非动画调用也能刷新单位图标，不再重复计算视野。
	unit_renderer.queue_redraw()
