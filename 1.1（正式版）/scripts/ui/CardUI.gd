...
func _ready() -> void:
	print("[CardUI] 手牌UI就绪")
	# 卡面是高密度像素画，使用最近邻采样，缩放时保持清晰的像素边缘。
...
func _process(delta: float) -> void:
	...
	hover_visual_lift = move_toward(hover_visual_lift, target_hover_lift, delta * 180.0)
...
func _get_card_layout(index: int, count: int, cs: Vector2) -> Dictionary:
	...
	var base_center := Vector2(
		_get_view_size().x * 0.5 + (float(index) - center_index) * (cs.x + _get_card_spacing()),
	...
	base_center.y -= lift
	return {"center": base_center, "rotation": deg_to_rad(normalized * CARD_ARC_ROTATION_DEGREES), "scale": scale}
...
func _is_point_in_card_layout(mouse_pos: Vector2, layout: Dictionary, cs: Vector2) -> bool:
	var local_pos: Vector2 = (mouse_pos - layout.center).rotated(-float(layout.rotation)) / float(layout.scale) + cs * 0.5
...
func _draw() -> void:
	...
	if selected_card_index >= 0 and selected_card_index < hand.size():
		_draw_card_at_layout(hand[selected_card_index], selected_card_index, _get_card_layout(selected_card_index, hand.size(), cs), cs)
	if hovered_card_index >= 0 and hovered_card_index < hand.size() and hovered_card_index != selected_card_index:
		_draw_card_at_layout(hand[hovered_card_index], hovered_card_index, _get_card_layout(hovered_card_index, hand.size(), cs), cs)
...
func _draw_help_bar(cs: Vector2) -> void:
	...
	if CardSystem.next_turn_command_penalty > 0:
		cp_text += "（待偿还%d）" % CardSystem.next_turn_command_penalty
	if CardSystem.next_turn_command_penalty > 0:
		cp_text += "（待偿还%d）" % CardSystem.next_turn_command_penalty
	draw_string(ThemeDB.fallback_font,
...
func get_card_at_pos(mouse_pos: Vector2) -> int:
	...
	var hand = CardSystem.hand
	var cs = _get_card_size()
	if hovered_card_index >= 0 and hovered_card_index < hand.size() and _is_point_in_card_layout(mouse_pos, _get_card_layout(hovered_card_index, hand.size(), cs), cs):
		return hovered_card_index
	if selected_card_index >= 0 and selected_card_index < hand.size() and selected_card_index != hovered_card_index and _is_point_in_card_layout(mouse_pos, _get_card_layout(selected_card_index, hand.size(), cs), cs):
		return selected_card_index
	for i in range(hand.size() - 1, -1, -1):
		if i == selected_card_index or i == hovered_card_index:
			continue
		if _is_point_in_card_layout(mouse_pos, _get_card_layout(i, hand.size(), cs), cs):
			return i
	return -1
