...
const STARTUP_PROLOGUE_SCRIPT = preload("res://scripts/ui/StartupPrologue.gd")
...
func _ready() -> void:
	...
	if GameManager.claim_startup_sequence():
		_build_startup_credits()
		_start_opening_sequence.call_deferred()
	else:
...
func _start_opening_sequence() -> void:
	startup_prologue = STARTUP_PROLOGUE_SCRIPT.new()
	startup_prologue.name = "StartupPrologue"
	startup_prologue.finished.connect(_on_startup_prologue_finished)
	add_child(startup_prologue)
	startup_prologue.play()


func _on_startup_prologue_finished() -> void:
	if startup_prologue != null:
		startup_prologue.queue_free()
		startup_prologue = null
	_play_startup_credits()


func _play_startup_credits() -> void:
	SoundManager.play_bgm("menu")
	if startup_credits != null:
		startup_credits.play()
