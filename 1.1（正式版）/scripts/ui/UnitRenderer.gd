...
func _draw_unit(unit: UnitBase, pos: Vector2) -> void:
	...
	# 烟雾本体支持悬停虚化，因此单位始终保持正常不透明度。
	var model_modulate := Color.WHITE
	...
	if not animation_config.is_empty():
		...
		var faded_faction := Color(faction_color.r, faction_color.g, faction_color.b, model_modulate.a)
		draw_circle(pos, radius, faded_faction)
		draw_circle(pos, radius, Color(1.0, 1.0, 1.0, model_modulate.a), false, max(1.0, ts * 0.03))
		...
		if icon != null:
			...
			draw_texture_rect(icon, rect, false, model_modulate)
...
func is_unit_visually_faded(unit: UnitBase) -> bool:
	"""兼容旧调用：烟雾中的单位不再虚化，改由鼠标悬停降低烟雾透明度。"""
	return false
