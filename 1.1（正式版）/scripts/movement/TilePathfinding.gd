...
static func find_path(from_col: int, from_row: int, to_col: int, to_row: int,
	...
	if not end_cell.is_passable_for(is_armored):
		return []
	if occupant_blocks_planned_movement(unit, end_cell.occupant_unit):
		return []
	...
	while not open_list.is_empty():
		...
		for nb in GridManager.get_neighbors(cur.col, cur.row):
			...
			if occupant_blocks_planned_movement(unit, cell.occupant_unit):
				continue
...
static func get_reachable_cells(unit: UnitBase) -> Array:
	...
	while not frontier.is_empty():
		...
		for nb in GridManager.get_neighbors(cur.x, cur.y):
			...
			var blocked_by_unit = occupant_blocks_planned_movement(unit, occ)
			if blocked_by_unit:
				continue
			...
			if not cost_map.has(key) or nc_cost < cost_map[key]:
				...
				if not result.has(Vector2i(nc, nr)):
					result.append(Vector2i(nc, nr))

	return result


static func occupant_blocks_planned_movement(moving_unit: UnitBase, occupant: UnitBase) -> bool:
	"""规划阶段只让玩家已知的单位阻挡蓝色范围；未知敌军在演绎接触时才使移动停止。"""
	if occupant == null or occupant == moving_unit:
		return false
	if moving_unit != null \
			and moving_unit.faction == UnitBase.Faction.WARSAW_PACT \
			and occupant.faction != moving_unit.faction \
			and FogOfWar.enabled \
			and not FogOfWar.is_unit_visible(occupant):
		return false
	return true
