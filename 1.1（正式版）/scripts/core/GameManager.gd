...
var startup_sequence_shown_this_session: bool = false # 每次程序运行只展示一次序章与制作组名单
...
func claim_startup_sequence() -> bool:
	"""首次主菜单领取启动流程；同一程序运行内后续主菜单实例直接跳过。"""
	if startup_sequence_shown_this_session:
		return false
	startup_sequence_shown_this_session = true
	return true
...
func _start_level_intro() -> void:
	...
	# 同步权威VP坐标回实际TileCell；此前只更新vp_cells数组，导致其中部分VP没有
	# marker，网格渲染器无法为它们绘制任务旗帜。
	for row in range(GridManager.MAP_HEIGHT):
		for col in range(GridManager.MAP_WIDTH):
			var map_cell = GridManager.get_cell(col, row)
			if map_cell != null and map_cell.marker == GridManager.CellMarker.VP_POINT:
				map_cell.marker = GridManager.CellMarker.NONE
	for vp_value in level_data.vp_cells:
		var vp := Vector2i(vp_value)
		var vp_cell = GridManager.get_cell(vp.x, vp.y)
		if vp_cell != null:
			vp_cell.marker = GridManager.CellMarker.VP_POINT
...
func load_game(slot: int) -> bool:
	...
	if not data is Dictionary or not data.has("level") or not data.has("units"):
		...
		return false
	# 存档结构版本严格校验 — 旧版(无 save_version)与未来版本一律拒绝, 防静默错读
	if not data.has("save_version") or int(data["save_version"]) != SAVE_VERSION:
		push_error("[GameManager] 存档槽位 %d 版本不兼容: 存档 save_version=%s, 当前要求 %d（旧档已作废, 请新建游戏）" \
			% [slot, str(data.get("save_version", "旧格式(无此字段)")), SAVE_VERSION])
		return false
