...
func apply_smoke(col: int, row: int, duration: int = 1, area_size: int = 4) -> void:
	"""施加正方形烟雾效果（「烟雾遮障」卡牌默认4×4）。"""
	var cells := _get_square_area_cells(col, row, area_size)
	for c in cells:
		var key = "%d,%d" % [c.x, c.y]
...
func execute_area_attack(attacker_id: int, center_col: int, center_row: int,
		area_size: int) -> Array[Dictionary]:
	"""对正方形区域进行面杀伤（呼叫炮击默认2×2）。"""
	var results: Array[Dictionary] = []
	var cells := _get_square_area_cells(center_col, center_row, area_size)
	for c in cells:
...
func execute_blind_fire(attacker_id: int, center_col: int, center_row: int,
		area_size: int = 3) -> Array[Dictionary]:
...
func _get_square_area_cells(center_col: int, center_row: int, area_size: int) -> Array[Vector2i]:
