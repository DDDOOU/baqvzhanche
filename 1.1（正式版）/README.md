...
正式版剧情主线与“坐标”主题设计参见：[《静默行动1987》剧情设计文档_v1.0.md](docs/《静默行动1987》剧情设计文档_v1.0.md)。
...
- 12张正式卡牌均使用 `assets/effects/card_effects/` 中的透明四帧像素序列帧；乱码牌与测试模板不绑定固定特效。
- 炮击、盲射、烟雾、布雷分别按2×2、3×3、4×4、1×2地块范围播放，其余目标型特效按1格播放。
- 动画在卡牌实际结算时播放；演绎阶段移动或缩放视角不会造成特效偏移。
- 如需从保留的美术母版重新生成清晰素材，在项目根目录执行 `python tools/build_card_effect_assets.py`。

## 当前可玩内容

- 当前十关均可从关卡选择进入，并可在胜利后连续进入下一关；第1、2关已有专用场景，第3至第10关使用可替换的功能地图框架。
- 第1关开局依次显示「开场剧情与对话」「战前剧情」「任务要求」；开场对话框位于画面下方并预留人物立绘空间。第2关沿用「战前剧情」「任务要求」两段式开场，第3至第10关使用传统任务简报面板。
