...
func _run() -> void:
	...
	_check(main.opening_dialogue_layer != null, "第一关应先显示开场剧情与对话")
	_check(main.intro_story_layer == null, "开场对白结束前不应提前显示战前剧情")
	var opening_dialogue = LevelDatabase.get_level(0).opening_dialogue
	_check(opening_dialogue.size() == 4, "第一关开场应包含字幕与三段人物对白")
	_check(_tree_contains_text(main.opening_dialogue_layer, "1987.09.14 / 04:30")
		and _tree_contains_text(main.opening_dialogue_layer, "最后确认位置：（4,5）"),
		"第一关开场首屏应显示时间、位置与无线电报告")
	var dialogue_style = main.opening_dialogue_panel.get_theme_stylebox("panel") as StyleBoxFlat
	_check(dialogue_style != null and is_equal_approx(dialogue_style.bg_color.a, 0.40),
		"开场对话框灰色背景应保持60%透明度")
	_check(main.opening_dialogue_portrait_left != null
		and main.opening_dialogue_portrait_right != null,
		"开场对话层应保留左右人物立绘空间")
	_check(main._opening_portrait_left_texture != null
		and not main._opening_portrait_left_texture.visible,
		"战区记录字幕不应误显示人物立绘")
	main._opening_dialogue_next_button.pressed.emit()
	await get_tree().process_frame
	_check(main._opening_portrait_left_texture.texture == CharacterPortraits.ANYA
		and main._opening_portrait_left_texture.visible,
		"第一关阿尼娅开场对白应显示对应立绘")
	_check(main._opening_dialogue_index == 1,
		"鼠标点击继续按钮应推进开场对白")
	_check(_tree_contains_text(main.opening_dialogue_layer,
		String(opening_dialogue[1].get("text", ""))),
		"鼠标点击后应显示第二段开场对白")
	for entry_index in range(2, opening_dialogue.size()):
		main._on_opening_dialogue_next_pressed()
		await get_tree().process_frame
		_check(main.opening_dialogue_layer != null, "开场对白播放完毕前不应提前进入战前剧情")
		_check(_tree_contains_text(main.opening_dialogue_layer,
			String(opening_dialogue[entry_index].get("text", ""))),
			"开场对白应按顺序显示第%d段" % (entry_index + 1))
	main._on_opening_dialogue_next_pressed()
	await get_tree().process_frame
	_check(main.opening_dialogue_layer == null and main.intro_story_layer != null,
		"开场对白结束后应进入战前剧情")
