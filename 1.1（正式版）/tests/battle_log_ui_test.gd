...
func _ready() -> void:
	...
	var story := StoryDialog.new()
	get_tree().root.add_child(story)
	await get_tree().process_frame
	story.queue_dialog("无线电", "车里有老人。——步兵班长")
	await get_tree().process_frame
	_check(story._portrait_left.texture == CharacterPortraits.SQUAD_LEADER
		and story._portrait_left.visible,
		"关卡内无线电署名应识别并显示前沿班长立绘")
	story.close_current()
	story.queue_dialog("铁砧指挥官", "保持推进。")
	await get_tree().process_frame
	_check(story._portrait_right.texture == CharacterPortraits.ANVIL_COMMANDER
		and story._portrait_right.visible,
		"北约角色对白应在右侧显示铁砧指挥官立绘")
	story.queue_free()

	if failures.is_empty():
		print("[BATTLE LOG UI TEST] PASS (8 checks)")
		get_tree().quit(0)
	else:
